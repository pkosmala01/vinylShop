...
class Seller
{
    ...
    vector <Section> thematic_sections; // inicjuje wektor z działaniami 
    bool status; // zajetosc pracownika 
public:
    Seller();
    Seller(string name1, string surname1, string login, string password, string per_level, vector<Section> them_sect); // konstruktor
    ...
    vector <Section> Get_thematic_sections(); // getter sekcji
    ...
    void Set_thematic_sections(Section section); // setter sekcji
    void Set_status(bool status1); // setter statusu
    Vinyl getVinyl(string name, string section);
